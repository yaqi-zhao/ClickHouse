...
public:
    ...
    ColumnPtr executeImpl(const ColumnsWithTypeAndName & arguments, const DataTypePtr &, size_t input_rows_count) const override
    {
        ...
        for (size_t i = 0; i < input_rows_count; ++i)
        {
            ...
            auto time = date_lut.makeDateTime(year, month, day, hour, minute, second);
            DataTypeDateTime::FieldType date_time;
            convertFromTime<DataTypeDateTime>(date_time, time);
...
public:
    ...
    ColumnPtr executeImpl(const ColumnsWithTypeAndName & arguments, const DataTypePtr &, size_t input_rows_count) const override
    {
        ...
        for (size_t i = 0; i < input_rows_count; ++i)
        {
            ...
            auto date_time = date_lut.makeDateTime(year, month, day, hour, minute, second);
            UInt64 fraction = fraction_data ? (*fraction_data)[i] : 0;
